...
interface Filter {
  ...
  limit: number;
  search: string;
  filters: typeof filterPanjarMuatanDetail;
  sortBy: string;
  sortDirection: 'asc' | 'desc';
}

const GridPanjarMuatanDetail = () => {
  const { theme, resolvedTheme } = useTheme();
  const isDark = theme === 'dark' || resolvedTheme === 'dark';
  const { user } = useSelector((state: RootState) => state.auth);
  const headerData = useSelector((state: RootState) => state.header.headerData);
  const panjarId = headerData?.id ? String(headerData.id) : '';
  ...
  const [rows, setRows] = useState<PanjarMuatanDetail[]>([]);
  ...
  const [filters, setFilters] = useState<Filter>({
    ...
    limit: 50,
    search: '',
    filters: { ...filterPanjarMuatanDetail },
    sortBy: 'nobukti',
    sortDirection: 'asc'
  });
  ...
  const [pageDataCache, setPageDataCache] = useState<
    Map<number, PanjarMuatanDetail[]>
  >(new Map());
  const streamBufferRef = useRef<Map<number, PanjarMuatanDetail[]>>(new Map());
  ...
  const { data: detail, isLoading } = useGetPanjarMuatanDetail(
    panjarId || undefined,
    queryParams
  );
    ...
    return [
      {
        ...
        renderCell: (props: any) => {
          // Nomor ABSOLUT: index dalam window + offset halaman pertama window,
          // supaya tidak mengulang dari 1 setiap kali window bergeser.
  ...
  }, [rows, filters, filters.filters, minVisiblePage]);
  ...
  const mapDetailRows = useCallback(
    (data: any[] | undefined | null): PanjarMuatanDetail[] =>
      (data ?? []).map((item: any) => ({
        id: item.id,
  ...
  const prefetchPages = useCallback(
    async (
      pagesToFetch: number[],
      existingCache?: Map<number, PanjarMuatanDetail[]>,
      ...
      await Promise.allSettled(
        validPages.map(async (pageNum) => {
          try {
            const data = await getPanjarMuatanDetailFn(panjarId, {
              ...queryParams,
              page: pageNum,
              limit: filters.limit
            });
  ...
  async function handleKeyDown(
    args: CellKeyDownArgs<PanjarMuatanDetail>,
  ...
  function handleCellClick(args: { row: PanjarMuatanDetail }) {
  ...
  function getRowClass(row: PanjarMuatanDetail) {
    const rowIndex = rows.findIndex((r) => r.id === row.id);
    return rowIndex === selectedRow ? 'selected-row' : '';
  }

  function rowKeyGetter(row: PanjarMuatanDetail) {
    return row.id;
  }
  ...
  useEffect(() => {
    ...
    const newCache = new Map<number, PanjarMuatanDetail[]>();
  ...
  useEffect(() => {
    const combinedRows: PanjarMuatanDetail[] = [];
  ...
  // Header berganti (user pindah baris master) = dataset benar-benar lain.
  // Buang seluruh window + buffer, jangan sampai detail panjar sebelumnya ikut
  // tergabung ke grid.
  useEffect(() => {
    setRows([]);
    setSelectedRow(0);
    resetBufferingCache();
  }, [panjarId, resetBufferingCache]);
  ...
  // Effect refetch manual saat `filters` berubah SENGAJA tidak ada: queryParams
  // ikut masuk ke query key react-query, jadi perubahan filter/sort/halaman
  // sudah otomatis memicu fetch. refetch() manual hanya menghasilkan request
    ...
    <div className={`flex h-[100%] w-full justify-center`}>
      <div className="flex h-[100%] w-full flex-col rounded-sm border border-border bg-background">
        <div className="flex h-[38px] w-full flex-row items-center justify-between rounded-t-sm border-b border-border bg-background-grid-header px-2">
          ...
          <div className="flex flex-row items-center">
            <DraggableColumn
              ...
              userId={user.id}
              gridName="GridPanjarMuatanDetail"
        ...
        <DataGrid
          ...
          // WAJIB false — lihat catatan yang sama di GridPanjarHeader.
...
export default GridPanjarMuatanDetail;
