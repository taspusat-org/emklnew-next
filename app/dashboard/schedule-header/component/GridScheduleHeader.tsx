...
import {
  ...
  generateScheduleExportFn,
  generateScheduleReportFn,
  getScheduleById,
  getScheduleDetailFn,
  getScheduleHeaderFn
...
const GridScheduleHeader = () => {
  ...
  const streamBufferRef = useRef<Map<number, ScheduleHeader[]>>(new Map());
  ...
  const forms = useForm<ScheduleHeaderInput>({
    resolver: mode === 'delete' ? undefined : zodResolver(scheduleHeaderSchema),
    mode: 'onSubmit',
    defaultValues: {
      nobukti: '',
      tglbukti: '',
  ...
  const { data: allData, isLoading: isLoadingData } = useGetScheduleHeader(
    {
      ...filters,
    ...
    return [
      {
        ...
        renderHeaderCell: (props: any) => (
          <div
            title={props.column.name.toUpperCase()}
            className="flex h-full w-full flex-col "
          >
            <div className="headers-cell h-[50%] w-full">
              <p className="w-full text-center text-sm font-normal">No.</p>
            </div>

            <div
              className={`h-[50%] w-[calc(100%+2px)] ${NOMOR_CELL_BOX} mb-1 items-center`}
            >
  ...
  function handleCellClick(args: { row: ScheduleHeader }) {
  ...
  const handleGoToLastPage = useCallback(async () => {
    ...
    try {
      const results = await Promise.all(
        pagesToFetch.map((p) =>
          getScheduleHeaderFn({ ...filters, page: p, limit: filters.limit })
        )
      );

      const newCache = new Map<number, ScheduleHeader[]>();
  ...
  const onSuccess = async (
    indexOnPage: number,
    fetchedPages: number[],
    pagedData: Record<string, ScheduleHeader[]>,
    pageNumber: number,
    keepOpenModal = false,
    ...
    try {
      ...
      if (mode !== 'delete') {
        ...
        const response = await api2.get(
          `/redis/get/scheduleheader-page-${pageNumber}`
        );
        const loadedRows: ScheduleHeader[] = Array.isArray(response.data)
          ? response.data
          : [];
        ...
        Object.entries(pagedData).forEach(([key, value]) => {
          updatedBuffer.set(Number(key), value as ScheduleHeader[]);
        });
  ...
  // LaporanSchedule.mrt adalah bukti per transaksi, bukan laporan daftar.
  ...
  const handleExportExcel = async (buktiId?: string) => {
    ...
    await generateExport({
      ...
      payload: { id: rowId },
      apiFn: generateScheduleExportFn
    });
  };
  ...
  const prefetchPages = useCallback(
    async (
      pagesToFetch: number[],
      existingCache?: Map<number, ScheduleHeader[]>,
      ...
      await Promise.allSettled(
        validPages.map(async (pageNum) => {
          try {
            const data = await getScheduleHeaderFn({
              ...filters,
              page: pageNum,
              limit: filters.limit
            });
  ...
  useEffect(() => {
    const combinedRows: ScheduleHeader[] = [];
  ...
  useEffect(() => {
    if (selectedRow !== null && rows.length > 0 && mode !== 'add') {
      ...
      forms.setValue('details', []); // Menyiapkan details sebagai array kosong jik
    }
